  ...
  <aside class="wb-sidebar" id="wb-sidebar">
    ...
    <div class="wb-sidebar-footer d-flex flex-column gap-2">
      ...
      <button id="wb-profile-btn" class="btn btn-sm text-start d-flex align-items-center gap-2 w-100 p-2 rounded-3" style="background:rgba(99,102,241,.08);border:1px solid rgba(99,102,241,.2);">
        <div id="wb-avatar" style="width:32px;height:32px;border-radius:10px;background:var(--wb-primary);display:flex;align-items:center;justify-content:center;font-weight:800;font-size:.75rem;color:#fff;flex-shrink:0;font-family:'Manrope',sans-serif;">U</div>
        <div class="text-start overflow-hidden">
          <div class="small fw-semibold text-truncate" id="wb-profile-name" style="max-width:140px;">My Account</div>
          <div style="font-size:.68rem;opacity:.55;" id="wb-profile-plan">Free Plan</div>
        </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body pt-3">
        ...
        <div class="text-center mb-4">
          <div id="modal-avatar" style="width:72px;height:72px;border-radius:20px;background:var(--wb-primary);display:flex;align-items:center;justify-content:center;font-weight:900;font-size:1.5rem;color:#fff;margin:0 auto 12px;font-family:'Manrope',sans-serif;">U</div>
          <h5 class="fw-bold mb-0" id="modal-profile-name">My Account</h5>
          <p class="small text-body-secondary mb-2" id="modal-profile-email">user@example.com</p>
          <span class="badge rounded-pill px-3 py-2" id="modal-plan-badge" style="background:rgba(99,102,241,.15);color:#6366f1;font-weight:700;"><i class="fas fa-seedling me-1"></i>Free Plan</span>
        </div>
        ...
        <div class="mb-3">
          ...
          <input type="text" id="profile-name-input" class="form-control" placeholder="Your name">
        </div>
        ...
        <div class="p-3 rounded-3 mb-3" style="background:rgba(99,102,241,.08);border:1px solid rgba(99,102,241,.2);">
          <div class="d-flex justify-content-between align-items-center">
            <div>
              <div class="small fw-bold" id="plan-name-text">Free Plan</div>
              <div class="small text-body-secondary" id="plan-desc-text">1 workspace · 10 tabs · 150hrs/mo</div>
            </div>
      ...
      <div class="modal-footer border-top-0 pt-0 justify-content-between">
        <button type="button" class="btn btn-sm btn-outline-danger" id="wb-logout-btn">
...
<script>
  ...
  // ── Auth Guard ──────────────────────────────
  (function () {
    if (!localStorage.getItem('wb_logged_in')) {
  ...
  $(function () {
    ...
    function loadProfile () {
      const email = localStorage.getItem('wb_user_email') || 'user@example.com';
      const name  = localStorage.getItem('wb_user_name')  || email.split('@')[0];
      const plan  = localStorage.getItem('wb_plan')        || 'free';

      const initials = name.split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 2);
      $('#wb-avatar, #modal-avatar').text(initials);
      $('#wb-profile-name, #modal-profile-name').text(name);
      $('#modal-profile-email').text(email);
      $('#profile-name-input').val(name);
      ...
      $('#wb-profile-plan').text(p.label);
      $('#modal-plan-badge').html('<i class="fas ' + p.icon + ' me-1"></i>' + p.label).css('color', p.color);
      $('#plan-name-text').text(p.label);
      $('#plan-desc-text').text(p.desc);

      try {
        const state = JSON.parse(localStorage.getItem('wb_state_v8') || '{}');
        $('#stat-folders').text((state.collections || []).length);
        $('#stat-tabs').text((state.websites || []).length);
      } catch(_) {}
    }

    $('#wb-profile-btn').on('click', function () { loadProfile(); modalProfile.show(); });

    $('#wb-save-profile').on('click', function () {
      const newName = $('#profile-name-input').val().trim();
      if (newName) {
        localStorage.setItem('wb_user_name', newName);
        loadProfile();
        modalProfile.hide();
      }
    });

    $('#wb-logout-btn').on('click', function () {
      if (confirm('Log out of Wibscreen?')) {
